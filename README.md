...
| **Primary LLM** | Google Gemini 2.0 Flash | Free tier: 15 RPM (requests per minute). Used for interpretation, clarification, signal analysis, and report generation. All pipeline steps use the same Flash model. |
...
- **Why not OpenAI?** — No free tier. Gemini 2.0 Flash is free and fast enough.
...
Browser                          Server                          Database
  ...
  │                                │                                │
  │── POST /api/pipeline/interpret-signals ▶ Gemini 2.0 Flash           │
  │◀── { dimension_insights[] } ────│                               │
  │                                │                                │
  ...
  │                                │                                │
  │── POST /api/pipeline/report ────▶ Gemini 2.0 Flash                 │
...
2. Sends all signals + founder's clarification answers to Gemini 2.0 Flash
...
2. Sends everything to Gemini 2.0 Flash with strict formatting rules
...
**LLM Config:** Gemini 2.0 Flash, temperature 0.5, max tokens 4,096
...
3. Maps results to signals with:
   - `signal_strength` based on result position (position ≤ 2 → 0.8, ≤ 5 → 0.6, else → 0.4)
...
3. Analyzes the trend direction based on growth percentage:
   - `growing` — growth > 20% (further split: >50% growth gets higher signal strength of 0.9)
...
**API:** Gemini 2.0 Flash / Groq fallback (no additional cost)

**How it works:**
1. Sends the structured idea summary to Gemini 2.0 Flash with **2 separate LLM calls** (temperature 0.3):
...
```
points > 100 OR comments > 50   → 0.9
points > 30  OR comments > 15   → 0.7
points > 10  OR comments > 5    → 0.5
else                             → 0.3
```
...
For LLM Knowledge signals:
```
with source_url       → 0.5
without source_url    → 0.35
```
...
| `POST /api/pipeline/interpret-signals` | 4 | Gemini 2.0 Flash | Returns dimension insights |
...
| `POST /api/pipeline/report` | 7 | Gemini 2.0 Flash | Returns full report |
